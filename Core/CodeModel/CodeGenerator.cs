...
namespace Pytocs.Core.CodeModel
{
    ...
    public class CodeGenerator
    {
            ...
            Func<IEnumerable<CodeMemberField>> valueGenerator)
        {
            ...
            foreach (var field in valueGenerator())
            {
                field.FieldName = ToSnakeCase(field.FieldName);
                if (!c.MemberNames.Add(field.FieldName)) continue;
                c.Members.Add(field);
            }
            return c;
        }
            ...
            Action bodyGenerator)
        {
            ...
            var list = baseClasses.Select(b => new CodeTypeReference(b));
            ...
            isInit = false;
            foreach (var field in fieldGenerator())
            {
                field.FieldName = ToSnakeCase(field.FieldName);
                if (!c.MemberNames.Add(field.FieldName)) continue;
                c.Members.Add(field);
        ...
        {
            if (c is CodeMemberField cc)
            {
                cc.FieldName = ToSnakeCase(cc.FieldName);
                if (!CurrentType.MemberNames.Add(cc.FieldName))
                    return;
            }
